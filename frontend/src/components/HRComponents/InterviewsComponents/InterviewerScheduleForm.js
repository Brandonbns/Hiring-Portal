...
class InterviewerScheduleForm extends Component {
  //constructor
  constructor(props) {
    super(props);
  ...
  setNumOfCandidates(event) {
    if (event.target.value) {
      this.error.numOfCandidates = "";
      this.setState({
  ...
  confirm() {
    ...
    if (
      ...
      axios({
        method: "post",
        url: environment.baseUrl + "/interviews/scheduleInterview",
        data: {
  ...
  requiredCheck() {
    ...
    interviewers.length === 0
      ...
      : (this.error.interviewers = "");
    !date ? (this.error.date = `date is required`) : (this.error.date = "");
    !startTime
  ...
  componentDidMount() {
    console.log(this.minTime, "min", this.maxTime, "max");
  ...
  //render functino
  render() {
    const handleDateChange = this.handleDateChange.bind(this);
    const setStartTime = this.setStartTime.bind(this);
    const setEndTime = this.setEndTime.bind(this);
    const interviewerChange = this.interviewerChange.bind(this);
    const locationChange = this.locationChange.bind(this);
    const scheduleInterview = this.scheduleInterview.bind(this);
    const changeData = this.changeData.bind(this);
    const confirm = this.confirm.bind(this);
    let disabled = this.state.scheduleDisabled;
    const locations = [
      {
        key: 1,
        value: "Head Office",
      ...
      <div>
        ...
        <div>
          <Modal
            ...
            onClose={() => this.setState({ scheduled: false })}
            onOpen={() => this.setState({ scheduled: true })}
            open={this.state.scheduled}
            size="tiny"
            scrolling={false}
          >
            ...
            <Modal.Content scrolling={false}>
              <div>
                ...
                to {this.state.endTime} at <b>{this.state.location}</b>
                <br />
                <br />
                the Interviewers are,
                {this.state.interviewers.map((Interviewer, index) => {
                  return <li key={index}>{Interviewer.Interviewer}</li>;
                })}
                <input
                  type="button"
                  value="Change"
...
export default InterviewerScheduleForm;

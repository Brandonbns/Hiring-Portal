...
const ViewPendingJobs = () => {
  ...
  const getJobs = () => {
    axios({
      method: "GET",
      url: env.baseUrl+"/jobs/getpendingjobs",
    })
      .then((response) => {
  ...
  const deleteJob = (index) => {
    axios({
      method: "POST",
      url: env.baseUrl+"/jobs/deletejobs",
      data: {
        job_id: index,
      },
    })
  ...
  const handleSubmit = (e) => {
    ...
    axios({
      ...
      .catch((err) => {
        console.log(err);
        alert("Oops can't complete the process!");
      });
  };

  return (
    <>
      <h3 className="header">You are viewing the pending jobs!</h3>
      ...
      <Modal show={show} onHide={() => setShow(false)}>
        <Modal.Header closeButton onClick={() => setShow(false)}>
          <Modal.Title>
            Heads Up! You are updating the pending jobs!
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default ViewPendingJobs;
